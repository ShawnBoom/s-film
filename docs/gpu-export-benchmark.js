import { createGpuExportBenchmarkRenderer } from "./gpu-preview.js?v=44";
...
export function formatGpuExportBenchmark(result, status = "Ready") {
  ...
  for (const test of result.cases) {
    ...
    lines.push(`CPU process/file: ${fixed(test.cpu.process)} / ${fixed(test.cpu.fileReady)} ms (${test.cpu.processor})`);
    lines.push(`GPU upload/LUT: ${fixed(test.gpu.upload)} / ${fixed(test.gpu.lut)} ms`);
    lines.push(`GPU submit/sync: ${fixed(test.gpu.submission)} / ${fixed(test.gpu.completion)} ms`);
    lines.push(`GPU read/file: ${fixed(test.gpu.readback)} / ${fixed(test.gpu.fileReady)} ms`);
...
export async function runGpuExportABBenchmark(options) {
  ...
  try {
    for (let index = 0; index < tests.length; index += 1) {
      ...
      onProgress({ phase: "case", index, total: tests.length, message: `Running ${test.key}: ${test.label}`, result });
      ...
      onProgress({ phase: "compare", index, total: tests.length, message: `Comparing ${test.key}: ${test.label}`, result });
      ...
      result.cases.push({
        ...
        label: test.label,
        edit: test.edit,
        cpu: { processor: cpuProcessor, sourceRead: cpuSourceRead, process: cpuProcess, putImageData: cpuEncoding.putImageData, jpeg: cpuEncoding.jpeg, jpegBytes: cpuEncoding.jpegBytes, fileReady: cpuFileReady },
        gpu: { sourceRead: gpuSourceRead, ...gpuTimings, putImageData: gpuEncoding.putImageData, jpeg: gpuEncoding.jpeg, jpegBytes: gpuEncoding.jpegBytes, fileReady: gpuFileReady },
        difference,
        compare,
      });
      cpuPixels = null;
      gpuPixels = null;
      onProgress({ phase: "case-complete", index, total: tests.length, message: `Completed ${test.key}: ${test.label}`, result });
      await nextTask();
    }
  } finally {
    renderer.destroy();
