import { S01_LUT, S01_LUT_SIZE } from "./s01-classic-neg-lut.js?v=48";
import { S02_LUT, S02_LUT_SIZE } from "./s02-classic-chrome-lut.js?v=48";
import { S03_LUT, S03_LUT_SIZE } from "./s03-classic-chrome-lut.js?v=48";
import { S04_LUT, S04_LUT_SIZE } from "./s04-pro400h-lut.js?v=48";
import { S05_LUT, S05_LUT_SIZE } from "./s05-superia400-lut.js?v=48";
import { S06_LUT, S06_LUT_SIZE } from "./s06-color100-lut.js?v=48";
import { S07_LUT, S07_LUT_SIZE } from "./s07-color800z-lut.js?v=48";
import { S08_LUT, S08_LUT_SIZE } from "./s08-gold-blue-lut.js?v=48";
import { S09_LUT, S09_LUT_SIZE } from "./s09-portra-cool-lut.js?v=48";
import { S10_LUT, S10_LUT_SIZE } from "./s10-proimage-original-lut.js?v=48";
import { S11_LUT, S11_LUT_SIZE } from "./s11-ektar100-lut.js?v=48";
import { S12_LUT, S12_LUT_SIZE } from "./s12-portra400-lut.js?v=48";
import { S13_LUT, S13_LUT_SIZE } from "./s13-gold200-lut.js?v=48";
import { S14_LUT, S14_LUT_SIZE } from "./s14-chrome64-lut.js?v=48";
...
export function getGrainParameters(grain, width, height) {
  ...
  const character = amount * amount * (3 - 2 * amount);
  const fineWeight = 0.8 - 0.12 * character;
  const mediumWeight = 0.17 + 0.07 * character;
  return {
    active: true,
    ...
    coarseScale: 5.2,
    clusterScale: 10.4,
    amplitude: 0.072 * amount ** 1.08,
    fineWeight,
    mediumWeight,
    coarseWeight: 1 - fineWeight - mediumWeight,
    clusterStrength: 0.008 + 0.017 * character,
  };
}

function shapeGrain(value) {
  return Math.sign(value) * Math.abs(value) ** 0.72;
}
...
function sampleGrain(r, g, b, x, y, parameters, seed) {
  ...
  const fine = shapeGrain(valueNoise(pointX, pointY, parameters.fineScale, seed));
  const medium = shapeGrain(valueNoise(
    pointX,
    pointY,
    parameters.mediumScale,
    seed ^ 0x45d9f3b,
  ));
  const coarse = valueNoise(pointX, pointY, parameters.coarseScale, seed ^ 0x27d4eb2d);
  const cluster = valueNoise(pointX, pointY, parameters.clusterScale, seed ^ 0x165667b1);
  const texture = (
