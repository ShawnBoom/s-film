...
test("ships cache-busted, relative GitHub Pages assets", async () => {
  ...
  assert.match(staticHtml, /type="module" src="\.\/app\.js\?v=30"/);
  assert.match(staticHtml, /href="\.\/styles\.css\?v=30"/);
  assert.match(staticApp, /from "\.\/image-engine\.js\?v=30"/);
  assert.match(staticWorker, /see-static-v30/);
  assert.match(staticWorker, /image-engine\.js\?v=30/);
  assert.match(staticWorker, /see-welcome\.png/);
  assert.match(appWorker, /see-v20/);
