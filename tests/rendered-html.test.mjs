...
test("ships cache-busted, relative GitHub Pages assets", async () => {
  ...
  assert.match(staticHtml, /type="module" src="\.\/app\.js\?v=51"/);
  assert.match(staticHtml, /href="\.\/styles\.css\?v=51"/);
  assert.match(staticApp, /from "\.\/image-engine\.js\?v=51"/);
  assert.match(staticApp, /from "\.\/lut-loader\.js\?v=51"/);
  assert.match(staticWorker, /see-core-v51/);
  assert.match(staticWorker, /see-luts-v51/);
  assert.match(staticWorker, /gpu-preview\.js\?v=51/);
  assert.match(staticWorker, /gpu-export\.js\?v=51/);
  assert.doesNotMatch(staticWorker, /gpu-export-benchmark/);
  assert.match(staticWorker, /export-processor\.js\?v=51/);
  assert.match(staticWorker, /export-worker\.js\?v=51/);
  assert.match(staticWorker, /image-engine\.js\?v=51/);
  assert.match(staticWorker, /edit-state\.js\?v=51/);
  assert.match(staticWorker, /lut-loader\.js\?v=51/);
  ...
  assert.equal(Array.from(staticLoader.matchAll(/\(\) => import\("\.\/s(?:0[1-9]|1[0-4])-[^"]+-lut\.js\?v=51"\)/g)).length, 14);
  ...
  assert.match(appWorker, /see-core-v51/);
  assert.match(appWorker, /see-runtime-v51/);
...
test("uses independent GPU preview and GPU-primary full-resolution export with CPU fallback", async () => {
  ...
  assert.match(gpuPreview, /uniform vec4 uGrainScales/);
  ...
  assert.match(engine, /1100 \/ Math\.max\(1, width, height\)/);
  assert.match(engine, /fineScale: 1\.35/);
  assert.match(engine, /mediumScale: 2\.6/);
  assert.match(engine, /coarseScale: 5\.2/);
  assert.match(engine, /clusterScale: 10\.4/);
  ...
  assert.match(exportWorker, /import \{ processPixels \} from "\.\/image-engine\.js\?v=51"/);
  assert.match(exportWorker, /import \{ loadFilterLut \} from "\.\/lut-loader\.js\?v=51"/);
...
test("shows actual preview and production export diagnostics only for debug query mode", async () => {
  ...
  assert.match(staticApp, /Grain v2\.1:/);
