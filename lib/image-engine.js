...
const GRAIN_PROFILE_A_KERNEL = Object.freeze([
  ...
  -0.002, 0.002, -0.02, 0.002, -0.002,
  -0.008, -0.02, 1, -0.02, -0.008,
  -0.002, 0.002, -0.02, 0.002, -0.002,
  -0.001, -0.002, -0.008, -0.002, -0.001,
]);

const GRAIN_PROFILE_B_KERNEL = Object.freeze([
  -0.001, -0.0015, -0.006, -0.0015, -0.001,
  -0.0015, 0.003, -0.01, 0.003, -0.0015,
  -0.006, -0.01, 1, -0.01, -0.006,
  -0.0015, 0.003, -0.01, 0.003, -0.0015,
  -0.001, -0.0015, -0.006, -0.0015, -0.001,
]);

const GRAIN_PROFILE_A_FIELD_NORMALIZATION = 1.00095355;
const GRAIN_PROFILE_B_FIELD_NORMALIZATION = 1.00030095;
const GRAIN_MICRO_VARIANCE_NORMALIZATION = 1.002;
const GRAIN_PROFILE_CORRELATION = 0.9995;
...
function grainMicroVariation(x, y, seed) {
  ...
  const scaleMix = 0.5 + 0.12 * Math.tanh(scaleSelector * 1.25);
  const densitySource = grainHashValue(x, y, seed ^ 0x63d83595);
  const density = 1 + 0.08 * Math.tanh(densitySource * 1.1);
