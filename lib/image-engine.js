...
const GRAIN_PROFILE_B_KERNEL = Object.freeze([
  -0.0035, -0.0055, -0.0165, -0.0055, -0.0035,
  -0.0055, 0.0035, 0.067, 0.0035, -0.0055,
  -0.0165, 0.067, 1, 0.067, -0.0165,
  -0.0055, 0.0035, 0.067, 0.0035, -0.0055,
  -0.0035, -0.0055, -0.0165, -0.0055, -0.0035,
]);

const GRAIN_PROFILE_A_FIELD_NORMALIZATION = 1.00308923;
const GRAIN_PROFILE_B_FIELD_NORMALIZATION = 1.00964598;
const GRAIN_MICRO_VARIANCE_NORMALIZATION = 1.0105;
const GRAIN_PROFILE_CORRELATION = 0.997;
...
function grainMicroVariation(x, y, seed) {
  const scaleSelector = grainHashValue(x, y, seed ^ 0xa511e9b3);
  const scaleMix = 0.55 + 0.15 * Math.tanh(scaleSelector * 1.25);
  const densitySeed = seed ^ 0x63d83595;
  const densitySource = (
    grainHashValue(x, y, densitySeed)
    + 0.35 * (
      grainHashValue(x - 1, y, densitySeed)
      + grainHashValue(x + 1, y, densitySeed)
      + grainHashValue(x, y - 1, densitySeed)
      + grainHashValue(x, y + 1, densitySeed)
    )
  ) / Math.sqrt(1 + 4 * 0.35 ** 2);
  const density = 1 + 0.18 * Math.tanh(densitySource * 1.1);
  return { scaleMix, density };
}
...
export function createReferenceCalibratedGrainField(width, height, seed = 1) {
  ...
  for (let y = 0; y < referenceHeight; y += 1) {
    for (let x = 0; x < referenceWidth; x += 1) {
      ...
      const center = excitation[y * referenceWidth + x];
      const variation = grainMicroVariation(x, y, seed);
      const filteredA = profileA / GRAIN_PROFILE_A_FIELD_NORMALIZATION;
      const filteredB = profileB / GRAIN_PROFILE_B_FIELD_NORMALIZATION;
      field[target] = (
        center + (filteredA - center) * variation.scaleMix
      ) * variation.density / GRAIN_MICRO_VARIANCE_NORMALIZATION;
      field[target + 1] = (
        center + (filteredB - center) * variation.scaleMix
      ) * variation.density / GRAIN_MICRO_VARIANCE_NORMALIZATION;
    }
  }
